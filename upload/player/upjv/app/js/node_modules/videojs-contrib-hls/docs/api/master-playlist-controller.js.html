...
import initializeMediaGroup from './media-groups';
...
export class MasterPlaylistController extends videojs.EventTarget {
  constructor(options) {
    ...
    this.audioGroups_ = { groups: {}, tracks: {}, activePlaylistLoader: null };
    this.subtitleGroups_ = { groups: {}, tracks: {}, activePlaylistLoader: null };
    this.ccGroups_ = { groups: {}, tracks: {} };
  ...
  setupMasterPlaylistLoaderListeners_() {
    this.masterPlaylistLoader_.on('loadedmetadata', () => {
      ...
      initializeMediaGroup('AUDIO', this.master(), this.audioGroups_, this);
      initializeMediaGroup('SUBTITLES', this.master(), this.subtitleGroups_, this);
      initializeMediaGroup('CLOSED-CAPTIONS', this.master(), this.ccGroups_, this);
  ...
  handleAudioinfoUpdate_(event) {
    ...
    for (let label in this.audioGroups_.tracks) {
      let track = this.audioGroups_.tracks[label];

      if (!enabledTrack &amp;&amp; track.enabled) {
        enabledTrack = track;
      }

      if (!defaultTrack &amp;&amp; track.properties_ &amp;&amp; track.properties_.default) {
        defaultTrack = track;
      }

      if (defaultTrack &amp;&amp; enabledTrack) {
        break;
      }
    }
