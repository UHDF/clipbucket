<?php
	...
	if ( (isset($_POST['thumb_time'])) && (isset($_POST['video_id'])) ){
		...
		if (is_array($thumbs)){
			$counter = count($thumbs)+1;		// Connaitre le prochain element
		}
		else{
			$counter = 1;
		}
		...
		if ((trim($max_video_file_by_size) <> '') and ($file_dir <> '')){
					...
					*/
					...
					foreach ($thumbs_settings_28 as $key => $thumbs_size) {
						...
						if ($key == 'original'){
							...
							$command = "curl --silent " . $data['remote_play_url'] . " | " . $ffmpeg_path." -i pipe:0 -ss ".$time." -f image2 -vframes 1 ".$output;
						...
						*/
						...
						foreach ($thumbs_settings_28 as $key => $thumbs_size) {
							...
							if ($key == 'original'){
								// Nom du fichier final
								$output = THUMBS_DIR.'/'.$file_name.'-original-'.$counter.'.jpg';
								$command = $ffmpeg_path." -i ".$data['remote_play_url']." -ss ".$time." -f image2 -vframes 1 ".$output;
							}
							else{
								$original = THUMBS_DIR.'/'.$file_name.'-original-'.$counter.'.jpg';
								...
								if (file_exists($original)){
									...
									$command = $ffmpeg_path." -i ".$data['remote_play_url']." -ss ".$time." -vf scale=".$width_setting.":".$height_setting." -an -r 1 -y -f image2 -vframes 1 ".$output;
								}
							}
